...
def analyze_toxicity(request):
    ...
    if request.method == "POST":
        ...
        if "lime_rating" in request.POST:
            lime_rating = int(request.POST.get("lime_rating", 3))  # Оценка LIME
            shap_rating = int(request.POST.get("shap_rating", 3))  # Оценка SHAP
            deeplift_rating = int(request.POST.get("deeplift_rating", 3))  # Оценка DeepLift
            integrated_gradients_rating = int(request.POST.get("integrated_gradients_rating", 3))  # Оценка IG

            # Сохраняем оценки в базу данных
            UserRating.objects.create(
                email=email or None,  # Если email пустой, сохраняем None
                text=text,
                lime_rating=lime_rating,
                shap_rating=shap_rating,
                deeplift_rating=deeplift_rating,
                integrated_gradients_rating=integrated_gradients_rating
            )
